...
function theme_almondb_frontpageblock02() {
    ...
    for ($i = 1, $j = 0; $i <= $count; $i++, $j++) {
        ...
        // Atribuição direta do ícone sem parsing.
        $templatecontext['block02'][$j]['icon'] = $theme->settings->$block02icon;
...
function theme_almondb_frontpageblock03() {
    ...
    for ($i = 1, $j = 0; $i <= $count; $i++, $j++) {
        ...
        $templatecontext['block03'][$j]['icon'] = format_string($theme->settings->$block03icon);
